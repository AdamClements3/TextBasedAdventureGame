	...
	switch(command){
		...
		case 2: //command for move north
				if(player.inHub && player.getNotes() == 1){
					...
					player.chooseFunctionFromMap(theMazes[currentMaze].checkRoomType(BG), theMazes[currentMaze], enemy);
				}
				else{
		...
		case 3: //command for move south
				if(player.inHub && player.getNotes() == 3){
					...
					player.inHub = false;
					Enemy Bosstert("Bosstert",2,10,100,0, "Heavy Constructive Criticism");
					Parser p2;
					player.fight(Bosstert, theMazes[currentMaze], p2);
				}
				else if(!player.inHub){
